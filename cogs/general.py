...
class General:
    ...
    @commands.command()
    async def rbxrap(self, username : str):
         """Gets the Rap from A ROBLOX User"""
         user = str.format(username)
         url = "http://roblox.plus:2052/inventory?username=" + user
         await self.bot.say("Fetching ROBLOX Rap Via ROBLOX+ API!")
         async with aiohttp.get(url) as r:
             result = await r.json()
             await self.bot.say(result['username'] + "'s Rap Is " + format(result['rap']))
